...
class GUI:
            ...
            try:
                ...
                if str(message) == "FILE":
                    ...
                    self.textCons.insert(
                        tk.END,
                        f"<{send_user}> {file_name} received\n\n"
                    )
                    self.textCons.config(state = tk.DISABLED) 
                    self.textCons.see(tk.END)
    ...
    def prettyPrinter(self,data_1):
         # List of stopwords 
        my_file = open("E:/dsa/CyberBull/Safe_Chat/stopwords.txt", "r")
        content = my_file.read()
        content_list = content.split("\n")
        my_file.close()
        tfidf_vector =  TfidfVectorizer(stop_words = content_list, lowercase = True,vocabulary=pickle.load(open("E:/dsa/CyberBull/Safe_Chat/tfidf_vector_vocabulary.pkl", "rb")))
        data_2=tfidf_vector.fit_transform([data_1])
        print(data_2)
        model = pickle.load(open("E:/dsa/CyberBull/Safe_Chat/LinearSVC.pkl", 'rb'))
